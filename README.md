...
- **Density Matrix (Pure State):** `ρ = |ψ⟩⟨ψ|`. For a qubit `|ψ⟩ = α|0⟩ + β|1⟩`: `ρ = [[|α|², αβ*], [α*β, |β|²]]`
...
```
ρ_A = Tr_B(ρ_AB) = (1/2)·I = [[1/2, 0], [0, 1/2]]
```
...
- **Rotation Gate U_rot,k:** Matrix `[[1, 0], [0, e^(2πi/2^k)]]`
...
```
L+ = √γ+ · [[0,1],[0,0]]    (emission: |1⟩→|0⟩)
L- = √γ- · [[0,0],[1,0]]    (absorption: |0⟩→|1⟩)
Lz = √γz · σz               (dephasing: no energy exchange)
```
